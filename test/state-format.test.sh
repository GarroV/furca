...
check_fixture() {
  ...
  # id обязан быть один на запись. Пишут в файлы состояния двое: сессия владельца
  # и диспетчер идущей стройки, а «следующий свободный номер» каждый берёт по
  # тому, что видит у себя. 06.09.2026 на живом проекте так появились два
  # D032, два D033, два T074 и два T075; git слил это без конфликта — строки
  # легли в разные места таблицы.
  #
  # Ловить обязана машина: ссылочная целостность после такого слияния остаётся
  # ЗЕЛЁНОЙ. Ссылки разрешаются — только не туда: `Q003 → D032` показывал уже на
  # чужое решение, и граф выглядел здоровым, пока владелец не прочитал текст.
  local dup_ids
  for pair in "$TASKS:T" "$QUESTIONS:Q" "$DECISIONS:D"; do
    local file="${pair%:*}" letter="${pair##*:}"
    dup_ids="$(grep -oE "^\| *${letter}[0-9]{3} *\|" "$file" | grep -oE "${letter}[0-9]{3}" \
      | sort | uniq -d || true)"
    if [[ -n "$dup_ids" ]]; then
      echo "FAIL: [$FIXTURE] ${file#$FIXTURE/}: один id занят дважды: $(echo "$dup_ids" | tr '\n' ' ')"
      echo "      Так выглядит запись двух сессий сразу: каждая взяла «следующий свободный»"
      echo "      по своему состоянию файла, а git слил строки без конфликта."
      echo "      Ссылки при этом разрешаются — но не туда, и проверка целостности молчит."
      echo "      Развести: более поздние записи переименовать и обновить все ссылки на них"
      echo "      (tasks.md, questions.md, decisions.md, progress.md, CHANGELOG.md)."
      exit 1
    fi
  done
